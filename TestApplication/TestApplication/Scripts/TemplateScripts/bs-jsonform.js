...
class JsonForm {
    ...
    _buildField(json, instance = "default") {
        ...
        switch (json.field.type) {
            case "list":
                template = `<div class="col-12 col-xs-12"><div id="` + id + `"></div></div>`
    ...
    _getFieldTemplate(id, json, enable_label = true, spacer_class = "mt-3") {
        ...
        switch (json.field.type) {
            ...
            case "multicheckbox":
                ...
                json.field.optionskeys.forEach((item, index) => {
                    ...
                    <div class="custom-control custom-checkbox">
                        <input type="checkbox" id="`+ id + index + `" name="` + id + `" value="` + item + `" class="custom-control-input" ` + isSelected + `>
            ...
            // Generate radios
            case "radio":
                template = `<div class="col-` + json.field.width + `"><div class="` + spacer_class + `">`
                json.field.optionskeys.forEach((item, index) => {
                    var isSelected = ""
                    if (json.field.default_value == item) {
                        isSelected = "checked"
                ...
                break
            case "inline-radio":
                template = `<div class="col-` + json.field.width + `" style="margin-top:20px">`
                json.field.optionskeys.forEach((item, index) => {
                    var isSelected = ""
                    if (json.field.default_value == item) {
                        isSelected = "checked"
...
`
            ...
            case "info-label":
                template = `<div class="col-` + json.field.width + `">
                 <span class="text-site font-weight-bold"><i class="fa fa-info-circle text-site"></i> ` + json.name + `</span>
                </div>           
                `
                break
            case "without-label-text":
                template = `
                <div class="col-`+ json.field.width + `">
                <div class="form-group `+ spacer_class + `">
                    <input type="`+ json.field.type + `" class="form-control ` + sizeClass + `" id="` + id + `">
                </div>
                </div>
                `
                break
            case "without-label-textarea":
                template = `
                <div class="col-`+ json.field.width + `">
                <div class="form-group `+ spacer_class + `">
                    <textarea class="form-control `+ sizeClass + `" id="` + id + `" rows="` + json.field.rows + `"></textarea>
                </div>
                </div>
                `
                break
            case "text-number":
                template = `<div class="col-` + json.field.width + `"><div class="input-group hd-mt-2" style="margin-top:5px">
            ...
            case "table":
                ...
                var tablerows = ""
                var column = ""
                json.field.headers.forEach((item, index) => {
                    tableheaders += `<th>`+item+`</th>`
                })
                json.field.tablerows.forEach((item, index) => {
                    column=""
                    for (var i = 0; i < item.row.length; i++) {
                        column+= `<td>` + item.row[i] + `</td>`
                    }
                    tablerows += `<tr>` + column+ `</tr>`
                })

        
                template = `<table class="table table-bordered table-striped table-responsive">
